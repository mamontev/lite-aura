...
function R:NormalizeWatchItem(item, unit)
  ...
  return {
    ...
    groupID = groupID,
      ...
      stackBehavior = (tostring(item.stackBehavior or "replace"):lower() == "add") and "add" or "replace",
      stackAmount = math.max(1, tonumber(item.stackAmount) or 1),
      maxStacks = math.max(1, tonumber(item.maxStacks) or 1),
    consumeBehavior = (tostring(item.consumeBehavior or "hide"):lower() == "decrement") and "decrement" or "hide",
