...
local legacyPresetGroups = {
  important_procs = true,
  defensives = true,
  target_debuffs = true,
}
...
local function normalizeTrackingMode(value, unit)
  ...
  unit = (((unitType == "string" or unitType == "number" or unitType == "boolean") and tostring(unit)) or "player"):lower()
...
function D:BuildEditableModel(entry)
  ...
  return {
    ...
    trackingMode = normalizeTrackingMode(item.trackingMode, entry.unit),
      ...
      produceTriggers = item.produceTriggers,
      duration = normalizeDuration(item.duration or item.estimatedDuration, 8),
      estimatedDuration = normalizeDuration(item.estimatedDuration, 8),
      timerBehavior = tostring(item.timerBehavior or "reset"),
      maxDuration = tonumber(item.maxDuration) or 0,
    stackBehavior = tostring(item.stackBehavior or "replace"),
    stackAmount = tonumber(item.stackAmount) or 1,
    maxStacks = tonumber(item.maxStacks) or 1,
    consumeBehavior = tostring(item.consumeBehavior or "hide"),
...
function D:BuildWatchItemFromModel(model, options)
  ...
  return {
    ...
    groupID = groupID,
      ...
      produceTriggers = model.produceTriggers,
      duration = normalizeDuration(model.duration or model.estimatedDuration, 8),
      estimatedDuration = normalizeDuration(model.estimatedDuration, 8),
      stackBehavior = (tostring(model.stackBehavior or "replace"):lower() == "add") and "add" or "replace",
      stackAmount = math.max(1, tonumber(model.stackAmount) or 1),
      maxStacks = math.max(1, tonumber(model.maxStacks) or 1),
      consumeBehavior = (tostring(model.consumeBehavior or "hide"):lower() == "decrement") and "decrement" or "hide",
...
function D:MigrateGroupLayoutState()
  ...
  for groupID in pairs(legacyPresetGroups) do
    if not referenced[groupID] and type(ns.db.groups[groupID]) == "table" then
      ns.db.groups[groupID] = nil
      if type(ns.db.positions) == "table" then
        ns.db.positions[groupID] = nil
      end
